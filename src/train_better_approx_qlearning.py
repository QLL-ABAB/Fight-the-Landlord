...
from douzero.rl.better_approx_qlearning import train
...
def configure_better_parser():
    for action in parser._actions:
        if action.dest == "feature_mode":
            action.default = "better_history"
            action.choices = ["better_history", "better_history_full", "history", "compact"]
            action.help = (
                "better_history is trimmed role-specific features; "
                "better_history_full keeps original history plus role-specific features; "
                "history/compact keep the original ApproxQ layouts"
            )
        elif action.dest == "savedir":
            action.default = "approx_qlearning_checkpoints/better_approxq"
...
def add_config_args():
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Use a named better_approxq training config from src/train_config.py",
    )
    parser.add_argument(
        "--list_configs",
        action="store_true",
        help="List named better_approxq training configs and exit",
    )
    parser.add_argument(
        "--no_feature_diag",
        action="store_true",
...
if __name__ == "__main__":
    ...
    if flags.list_configs:
        for name, config in sorted(train_configs_for_algorithm("better_approxq").items()):
            print(config_summary(config))
        raise SystemExit(0)

    if flags.config:
        config = get_train_config(flags.config, algorithm="better_approxq")

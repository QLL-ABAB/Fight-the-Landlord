...
class BayesianMDPAgent(object):
    ...
    def __init__(self, position, debug=False):
        ...
        self.cfg = {
            ...
            "bomb_prob_cap": 0.65,
            # --------------------------------------------------------
            # 3. 主动出牌评分参数 choose_leading_action
            # --------------------------------------------------------
            "hand_improve_weight": 10.0,
            "action_len_weight": 1.2,
            "min_card_bonus": 8.0,
            "single_rank_penalty": 0.35,
            "control_cost_weight": 1.8,
            "leading_bomb_penalty": 60.0,
            "bomb_near_finish_offset": 2,
            "bomb_near_finish_bonus": 40.0,
            "risk_penalty_weight": 4.0,
            # --------------------------------------------------------
            # 4. 主动牌型奖励 leading_type_bonus
            # --------------------------------------------------------
            "empty_action_penalty": -9999.0,
            "chain_base_bonus": 12.0,
            "chain_len_bonus": 0.8,
            "trio_bonus": 8.0,
            "pair_bonus": 4.0,
            "four_with_penalty": -8.0,
            # --------------------------------------------------------
            # 5. 跟牌策略参数 choose_following_action
            # --------------------------------------------------------
            ...
            # --------------------------------------------------------
            # 9. choose_lowest_cost_action 动作成本参数
            # --------------------------------------------------------
            "cost_len_weight": 0.5,
            "cost_rank_weight": 0.08,
            "cost_bomb_weight": 50.0,
            # --------------------------------------------------------
            # 10. control_cost 控制牌成本参数
            # --------------------------------------------------------
            "A_cost": 1.5,
            "2_cost": 4.5,
            "B_cost": 7.0,
    ...
    def act(self, infoset):
        """
        输入：infoset
        输出：infoset.legal_actions 中的一个合法动作
        """
        try:
    ...
    def estimate_bomb_prob(self, unknown_counter):
        ...
        return min(
            self.cfg["bomb_prob_cap"], possible * self.cfg["bomb_prob_per_possible"]
        )
    ...
    def choose_leading_action(self, legal_actions, state, belief):
        ...
        for action in candidates:
            ...
            # 核心：出完之后手牌更好，才是好动作
            score += self.cfg["hand_improve_weight"] * (current_badness - next_badness)

            # 出牌数量有奖励，但不能压过手牌结构判断
            score += self.cfg["action_len_weight"] * len(action)
            ...
            # 不要主动用高单张开路
            if len(action_str) == 1:
                score -= self.cfg["single_rank_penalty"] * self.main_rank_value(
                    action_str
                )

            # 控制牌不要乱出
            score -= self.cfg["control_cost_weight"] * self.control_cost(
                action_str, state
            )

            # 炸弹和王炸，主动出牌时除非能大幅改善手牌，否则强烈惩罚
            if self.is_bomb_or_rocket(action_type):
                score -= self.cfg["leading_bomb_penalty"]

                if (
                    len(action)
                    >= state["my_count"] - self.cfg["bomb_near_finish_offset"]
                ):
                    score += self.cfg["bomb_near_finish_bonus"]

            # Bayesian 风险只作为轻微惩罚
            risk = self.estimate_beaten_risk(action_str, action_type, belief)
            score -= self.cfg["risk_penalty_weight"] * risk
    ...
    def leading_type_bonus(self, action_str, action_type):
        ...
        if n == 0:
            return self.cfg["empty_action_penalty"]
        ...
        if self.is_chain_type(type_str):
            bonus += self.cfg["chain_base_bonus"] + self.cfg["chain_len_bonus"] * n

        # 三带、三张通常比单牌好
        if "trio" in type_str or "three" in type_str or max(counts.values()) == 3:
            bonus += self.cfg["trio_bonus"]

        # 对子比单牌略好
        if n == 2 and max(counts.values()) == 2:
            bonus += self.cfg["pair_bonus"]

        # 四带二不一定好，容易拆炸弹，略扣
        if max(counts.values()) == 4 and n > 4:
            bonus += self.cfg["four_with_penalty"]
    ...
    def estimate_beaten_risk(self, action_str, action_type, belief):
        ...
        if self.is_bomb_or_rocket(type_str):
            ...
            return min(
                self.cfg["risk_cap"],
                belief["rocket_prob"]
                + self.cfg["higher_bomb_risk_weight"] * higher_bombs,
            )
    ...
    def choose_lowest_cost_action(self, actions):
        ...
        for action in actions:
            ...
            cost = 0.0
            cost += self.cfg["cost_len_weight"] * len(action)

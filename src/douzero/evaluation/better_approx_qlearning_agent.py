...
from douzero.rl.better_approx_qlearning import (
    DEFAULT_BETTER_APPROX_Q_DIR,
...
def resolve_better_approx_q_model_path(model_path=None):
    if model_path:
        ...
        task_dir = os.path.join(DEFAULT_BETTER_APPROX_Q_DIR, model_path)
        latest = _latest_pkl_in_dir(task_dir)
        if latest:
            return latest
        return model_path

    if os.path.isfile(DEFAULT_BETTER_APPROX_Q_PATH):
        return DEFAULT_BETTER_APPROX_Q_PATH

    latest = _latest_pkl_in_dir(DEFAULT_BETTER_APPROX_Q_DIR)
    if latest:
        return latest

    raise FileNotFoundError(
        "No better approximate Q-learning checkpoint found. Use "
        "better_approxq:/path/model.pkl or train one under {}".format(
            DEFAULT_BETTER_APPROX_Q_DIR
        )
    )


class BetterApproxQLearningAgent(object):
    def __init__(self, position, model_path=None, device="cpu"):
        self.name = "BetterApproxQLearning"
        self.position = position
        self.model_path = resolve_better_approx_q_model_path(model_path)
        self.model = BetterApproxQModel.load(self.model_path, device=device)

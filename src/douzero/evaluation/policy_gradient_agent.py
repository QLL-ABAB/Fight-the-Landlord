...
DEFAULT_POLICY_DIR = "policy_gradient_checkpoints/policy_gradient"
...
def resolve_policy_gradient_model_path(model_path=None):
    if model_path:
        ...
        task_dir = os.path.join(DEFAULT_POLICY_DIR, model_path)
        latest = _latest_pkl_in_dir(task_dir)
        if latest:
            return latest
        return model_path

    if os.path.isfile(DEFAULT_POLICY_PATH):
        return DEFAULT_POLICY_PATH

    latest = _latest_pkl_in_dir(DEFAULT_POLICY_DIR)
    if latest:
        return latest

    raise FileNotFoundError(
        "No policy-gradient checkpoint found. Use pg:/path/model.pkl "
        "or train one under {}".format(DEFAULT_POLICY_DIR)
    )
...
class PolicyGradientAgent(object):
    def __init__(self, position, model_path=None):
        ...
        self.model_path = resolve_policy_gradient_model_path(model_path)
        self.model = LinearPolicyGradientModel.load(self.model_path)

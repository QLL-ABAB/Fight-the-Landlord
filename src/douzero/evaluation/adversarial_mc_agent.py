...
# ------------------------------------------------------------
# Adversarial Dou Dizhu agent with the high-ranked Botzone decision logic
# integrated as its internal depth-1 policy and rollout evaluator.
#
# Goal:
#   Keep the original implementation logic as the internal high-rank decision
#   core, while exposing a single public agent:
#       agent = AdversarialSearchAgent(position="landlord")
...
class _HighRankDecisionCore:
    """
    Internal Python version of the high-ranked Botzone C++ decision core.

    AdversarialSearchAgent uses this core for depth=1 behavior, root action
    generation, and leaf MCTS value estimation.
    ...
    def __init__(self, position: str, debug: bool = False, time_limit_sec: float = 0.95, n_root_actions: int = 3, seed: Optional[int] = None):
        self.name = "AdversarialHighRankCore"
...
#   depth == 1: exactly use the integrated high-rank Monte Carlo behavior.
#   depth  > 1: run a shallow adversarial/team search; when the search reaches
#               a leaf, evaluate that leaf by the integrated high-rank Monte Carlo
#               rollout (MCTS method) from that leaf state.
#
# This preserves the high-rank logic as the depth-1 base case, while making deeper
# settings a genuine adversarial search over explicit next actions.
# ---------------------------------------------------------------------------

class AdversarialSearchAgent(_HighRankDecisionCore):
    """
    Single public adversarial agent.

    It uses the integrated high-rank Monte Carlo core at depth=1, and switches
    to explicit adversarial/team search with high-rank MCTS leaf values when
    search_depth > 1.
    ...
    Semantics:
      - search_depth <= 1:
            use the integrated high-rank Monte Carlo action.
      - search_depth > 1:

...
parser = argparse.ArgumentParser(
    description="DouZero: policy-gradient training with an opponent pool"
)

parser.add_argument("--episodes", default=10000, type=int,
                    help="Number of training episodes")
parser.add_argument("--name", default="pg_pool_logadp_10k_history", type=str,
...
parser.add_argument("--savedir",
                    default="policy_gradient_checkpoints/policy_gradient_pool",
                    help="Root directory for opponent-pool checkpoints")
...
parser.add_argument("--learning_rate", default=0.002, type=float,
                    help="REINFORCE step size for linear policy weights")
parser.add_argument("--gamma", default=0.98, type=float,
                    help="Discount factor for reward-to-go")
parser.add_argument("--baseline_decay", default=0.99, type=float,
                    help="Moving-average baseline decay per position")
parser.add_argument("--l2", default=0.00001, type=float,
                    help="Small L2 shrinkage for linear weights")
parser.add_argument("--grad_clip", default=5.0, type=float,
                    help="Clip policy-gradient L2 norm; 0 disables clipping")
...
parser.add_argument("--feature_mode", default="history", type=str,
                    ...
                    help="Reuse approximate-Q encoded linear features")
...
parser.add_argument("--train_positions", default="all", type=str,
                    help="'all' or comma-separated positions to update")
parser.add_argument("--opponent_pool", default="current,rlcard,heuristic,random",
                    type=str,
                    help="Comma-separated opponent methods; supports current and pg:path")
parser.add_argument("--opponent_weights", default="", type=str,
                    help="Optional comma-separated sampling weights matching opponent_pool")
parser.add_argument("--current_opponent_weight", default=1.0, type=float,
                    help="Weight used for 'current' entries when opponent_weights is omitted")
parser.add_argument("--fixed_current_position", default="", type=str,
                    choices=["", "landlord", "landlord_up", "landlord_down"],
                    help="If set, only train current model in this role")

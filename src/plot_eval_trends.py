...
def plot_rows(path: Path, rows, baseline_win_rate: float, test_role: str):
    ...
    for series_name in ("approxq", "douzero"):
    ...
    model_rows = [row for row in rows if row["series"] in {"approxq", "douzero"}]
